...
"BT");f.TM=function(a,d,b,e){a.info[e]=b[0];d.BL=b[0];d.WL=b[0]};var l=/\(|\)|(;(\s*[A-Za-z]+\s*((\[\])|(\[(.|\s)*?(([^\\]|[^\\]\\\\)\])))+)*)/g,d=/[A-Z]+\s*((\[\])|(\[(.|\s)*?(([^\\]|[^\\]\\\\)\])))+/g,k=/[A-Z]+/,a=/(\[\])|(\[(.|\s)*?(([^\\]|[^\\]\\\\)\]))/g;b.SGF.parse=function(h){var f=[],g,e,c,q=new b.Kifu,r=null;h=h.match(l);for(var p in h)if("("==h[p])f.push(r);else if(")"==h[p])r=f.pop();else{r&&q.nodeCount++;r=r?r.appendChild(new b.KNode):q.root;g=h[p].match(d)||[];q.propertyCount+=g.length;
